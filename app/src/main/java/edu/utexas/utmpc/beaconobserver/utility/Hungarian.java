...
public class Hungarian {
    ...
    private void colorNeighbors(int row, int col, int maxVH) {
        ...
        for (int i =
             0; i < values.length; i++) { // Loop on cell at indexes [row][col] and its neighbors
            if (maxVH > 0)    // if value of maxVH is positive, color vertically
                lines[i][col] =
                        lines[i][col] == -1 || lines[i][col] == 2 ? 2 : 1; // if cell was colored before as horizontal (-1), and now needs to be colored vertical (1), so this cell is an intersection (2). Else if this value was not colored before, color it vertically
            else            // if value of maxVH is zero or negative color horizontally
                lines[row][i] =
                        lines[row][i] == 1 || lines[row][i] == 2 ? 2 : -1; // if cell was colored before as vertical (1), and now needs to be colored horizontal (-1), so this cell is an intersection (2). Else if this value was not colored before, color it horizontally
        }
...
//		printMatrix(lines); // Monitor the line draw steps
    ...
    public void createAdditionalZeros() {
        int minUncoveredValue =
                0; // We don't know the value of the first uncovered cell, so we put a joker value 0 (0 is safe, because before this step, all zeros were covered)
    ...
    private boolean optimization(int row) {
        ...
        for (int col = 0; col < values.length; col++) { // Try all columns
            if (values[row][col] == 0 && occupiedCols[col] == 0) { // If the current cell at column `col` has a value of zero, and the column is not reserved by a previous row
                ...
                if (optimization(
                        row + 1)) // If the next rows were assigned successfully a cell from a unique column, return true
                    return true;
                occupiedCols[col] =
                        0; // If the next rows were not able to get a cell, go back and try for the previous rows another cell from another column
            }
        }
